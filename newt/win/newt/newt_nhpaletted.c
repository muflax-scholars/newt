...
#ifdef CHANGE_COLOR
void
newt_change_colour(colour,rgb,reverse)
    int colour;
    long rgb;
    int reverse;
{
#ifdef DEBUG
    printf("- newt_change_colour(%d, %06X, %d);\n",
        colour, rgb, reverse);
#endif
...
#ifdef CHANGE_COLOR
char *
newt_get_colour_string(void)
{
#ifdef DEBUG
    printf("- newt_get_colour_string();\n");
#endif
...
#ifdef MAC
void *
newt_change_background(white)
    int white;
{
#ifdef DEBUG
    printf("- newt_change_background(%s);\n",
        white ? "white" : "black");
#endif
...
#ifdef MAC
short
newt_set_font_name(wintype, font_name)
    int wintype;
    char *font_name;
{
#ifdef DEBUG
    printf("- newt_set_font_name(");
    switch (wintype) {
        case NHW_MESSAGE:
            printf("NHW_MESSAGE");
            break;
